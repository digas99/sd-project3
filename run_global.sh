...
run_servers() {
    xterm -T "AssaultPartyA" -hold -e "sshpass -f password ssh $ASSAULTPARTYAHOST 'fuser -k $ASSAULTPARTYAPORT/tcp; ./run_server.sh AssaultParty $ASSAULTPARTYAPORT localhost $((ASSAULTPARTYAPORT+1)) /lib/genclass.jar'" &
    xterm -T "AssaultPartyB" -hold -e "sshpass -f password ssh $ASSAULTPARTYBHOST 'fuser -k $ASSAULTPARTYBPORT/tcp; ./run_server.sh AssaultParty $ASSAULTPARTYBPORT localhost $((ASSAULTPARTYBPORT+1)) /lib/genclass.jar'" &
    xterm -T "CollectionSite" -hold -e "sshpass -f password ssh $COLLECTIONSITEHOST 'fuser -k $COLLECTIONSITEPORT/tcp; ./run_server.sh CollectionSite $COLLECTIONSITEPORT localhost $((COLLECTIONSITEPORT+1)) /lib/genclass.jar'" &
    xterm -T "ConcentrationSite" -hold -e "sshpass -f password ssh $CONCENTRATIONSITEHOST 'fuser -k $CONCENTRATIONSITEPORT/tcp; ./run_server.sh ConcentrationSite $CONCENTRATIONSITEPORT localhost $((CONCENTRATIONSITEPORT+1)) /lib/genclass.jar'" &
    xterm -T "Museum" -hold -e "sshpass -f password ssh $MUSEUMHOST 'fuser -k $MUSEUMPORT/tcp; ./run_server.sh Museum $MUSEUMPORT localhost $((MUSEUMPORT+1)) /lib/genclass.jar'" &
}
